...
public class FSampleAdapter extends RecyclerView.Adapter {
    private Context context;//上下文
    private LayoutInflater layoutInflater;//动态加载布局
    private List<SampleBean> list;//保存要显示的数据
    ...
    //1自定义：构造方法，传进上下文
    public FSampleAdapter(Context context) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        final SampleBean sampleBean = list.get(position);
        if (sampleBean == null)
            return;
